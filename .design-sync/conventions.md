...
**Three full-screen shells hold member navigation state, and none of them draws a control the source doesn't spell out.** `OnboardingScreen`/`OnboardingStep` compose the eleven-step arc — deliberately no progress bar, no step counter, no back control, ever; the primary action is a slot (`action={<Button .../>}`), not a variant, because the two source canvases disagree on its fill and neither is picked here. `TaskScreen`/`TaskDone` are a single flow's in-progress and completed screens — pass `onExit` and real content, never invent a header. `PortalShell` composes the member portal chrome from its own parts (`PortalNav`, `PortalConversation(Header/Footer)`, `PortalPane(Body/Title)`, `PortalIdentity`, `PortalDestination`, `PortalBack`, `PortalMessageBar`, `PortalBody`) — read `PortalShell.prompt.md` for the `view` prop before composing a new portal layout; the parts are not meant to be placed outside a `PortalShell`.
